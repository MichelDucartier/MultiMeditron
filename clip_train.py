...
class DataTrainingArguments:
    ...
    use_fsdp: bool = field(
        default=False, metadata={"help": "Whether to use FullyShardedDataParallel for training."}
    )

    fsdp_cpu_offload: bool = field(
        default=False, metadata={"help": "Whether to offload the optimizer states to CPU when using FSDP."}
    )
...
def get_combined_dataset(dataset_configs: List[DatasetConfig], model_args: ModelArguments):
    ...
    for config in dataset_configs:
        ...
        if "train" in dataset:
            dataset["train"] = dataset["train"].rename_column(config.image_column, "image_path")
            dataset["train"] = dataset["train"].rename_column(config.caption_column, "caption")
...
def main():
    ...
    if training_args.do_train:
        ...
        logger.info(f"model: {config.vision_config.image_size}")
