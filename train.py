...
def build_datasets(config):
    ...
    print(f"Detected {num_proc} CPU cores, using all for dataset processing.")
...
parser.add_argument("-c", "--config", help="Path of the MultiMeditron configuration")
...
tokenizer = AutoTokenizer.from_pretrained(config["base_llm"], padding_side='right', use_fast=True)
...
with deepspeed.zero.Init(dtype=torch.bfloat16):
    if config.get("base_model", None) is None:
        model = bootstrap(config, tokenizer, attachment_token_idx, modalities_config)
    else:
        ...
        model = MultiModalModelForCausalLM.from_pretrained(config["base_model"], config=multimodal_config)
...
trainer = MultimodalTrainer(
        ...
        training_mode=TRAINING_MAPPING[config["training_mode"]],
        pytorch_profiler_config=config.get("pytorch_profiler", None),
        callbacks=trainer_callbacks,
)

if torch.distributed.get_rank() ==0:
    run = wandb.init(project="MultiMeditron", config = config ,name = config["training_args"]["run_name"])

    import json
    with open(config["training_args"]["deepspeed"], "r") as ds_file:
